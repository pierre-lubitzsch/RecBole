...
def create_dataset(config, unlearning=False, spam=False):
    ...
    # Check if task_type is NBR - use NextBasketDataset
    if config.get("task_type") == "NBR":
        dataset_class = getattr(dataset_module, "NextBasketDataset")
...
def data_preparation(config, dataset, unlearning=False, spam=False, sampler=None):
    ...
    """
    dataloaders = load_split_dataloaders(config) if not unlearning else None
    if dataloaders is not None:
        ...
        if model_type != ModelType.KNOWLEDGE:
            train_data = get_dataloader(config, "train", unlearning=unlearning)(
